  ...
  if (formatoAtom) {
    ...
    noticias = Array.from(valores).map(valor => {
      ...
      return {
        ...
        dataDePublicacao: valor.querySelector('updated')?.textContent || valor.querySelector('published')?.textContent || new Date().toISOString(),
        categoria: Array.from(valor.querySelectorAll('category')).map(c => c.getAttribute('term') || c.textContent).filter(Boolean),
      };
    });
  } else {
    ...
    noticias = Array.from(elementosnoticias).map(item => {
      ...
      return {
        ...
        categoria: Array.from(item.querySelectorAll('category')).map(c => c.textContent).filter(Boolean),
      };
    });
  }
